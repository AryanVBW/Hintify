...
class Overlay:
    def __init__(self, root):
        ...
        self.top.title("SnapAssist AI - Hints")
        self.top.attributes("-topmost", True)
        self.top.attributes("-alpha", 0.95)
        self.top.geometry("+100+100")
        self.top.configure(bg="white")

        frame = tk.Frame(self.top, bg="white", padx=10, pady=10)
        frame.pack()

        self.text_widget = tk.Text(frame, wrap="word", bg="white", fg="black", font=("Helvetica", 16))
        self.text_widget.config(state="disabled")
        self.text_widget.pack()

        self.top.bind("<Escape>", lambda e: self.top.withdraw())
        self.top.protocol("WM_DELETE_WINDOW", self.top.withdraw)  # <-- Prevent destruction
        self.top.withdraw()

    def show(self, response):
        if not self.text_widget.winfo_exists():  # <-- Safe check
            return
...
def gui_loop():
    ...
    def poll_queue():
        ...
        while not response_queue.empty():
            ...
            overlay.show(response)
        # Schedule next poll instead of calling itself directly
